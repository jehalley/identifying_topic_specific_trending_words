...
query = "SELECT word, count_per_day_all, total_word_count_per_day_all, sub_freq_to_all_freq_ratio FROM reddit_results WHERE '[2017-01-01, 2017-02-01]'::daterange @> date  ;"
cursor = connection.cursor()
cursor.execute(query)
data = cursor.fetchall()
query_data = pd.DataFrame(data = data, columns=['word', 'count_per_day_all','total_word_count_per_day_all', 'sub_freq_to_all_freq_ratio'])
connection.close()
...
query_data_no_stops['frequency'] = query_data_no_stops['sub_freq_to_all_freq_ratio']

average_specificity = pd.DataFrame(query_data_no_stops.groupby('word')['sub_freq_to_all_freq_ratio'].mean())

top_50th_percentile_words = average_specificity[average_specificity.sub_freq_to_all_freq_ratio< average_specificity.sub_freq_to_all_freq_ratio.quantile(.50)]


average_specificity.to_csv('average_word_specificity.csv')
